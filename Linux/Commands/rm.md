...
> ```
> Ищут такие файлы через `lsof +L1` или по пометке **`(deleted)`** — подробности в заметке про [lsof](lsof.md). Не помогает и `rm` от root — помогает `> /proc/<pid>/fd/N` или перезапуск процесса.
