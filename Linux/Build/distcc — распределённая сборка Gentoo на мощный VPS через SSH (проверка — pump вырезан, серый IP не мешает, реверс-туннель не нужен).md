...
`dev-util/ccache` 4.13.5, `FEATURES="ccache"`. Ускоряет **повторную** сборку того же кода, distcc — первую. Работают вместе, порядок важен: сначала ccache, потом distcc, иначе кэш будет промахиваться. На Gentoo связку включает сам Portage при обоих FEATURES.
...
Разумный порядок действий: сначала `ccache` (бесплатно и без рисков), потом **binhost** (закрывает всё), и только если хочется именно распределённой сборки в реальном времени — distcc.
