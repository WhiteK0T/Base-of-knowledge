...
> - **Лицензия не указана** (файла `LICENSE` в репозитории нет) — по умолчанию все права у автора; учитывай это перед использованием в своих проектах.

> [!note] Оригинал и форк
> `y13sint/FreeQwenApi` — **оригинал** (создан в мае 2025, ~218★, активно поддерживается). `ForgetMeAI/FreeQwenApi` — **форк** (создан в июне 2026, ~126★), добавивший свой брендинг и водяной знак `t.me/forgetmeai`. Автор оригинала **влил доработки форка обратно** к себе — поэтому в README оригинала есть раздел «Возможности fork» и водяной знак `t.me/forgetmeai` (в ответах API и health-метаданных может встречаться `"watermark": "t.me/forgetmeai"`). Файла `LICENSE` в репозитории нет — **лицензия не указана**.
