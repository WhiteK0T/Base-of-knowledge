...
- **Git:** [git status — состояние рабочего дерева](VCS/Git/status.md) · [git diff — просмотр изменений](VCS/Git/diff.md) · [git add — индексация изменений](VCS/Git/add.md) · [git commit — фиксация изменений](VCS/Git/commit.md) · [git log — история коммитов](VCS/Git/log.md) · [git reset — откат указателя и индекса](VCS/Git/reset.md)
